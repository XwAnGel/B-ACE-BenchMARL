...
class Mlp(Model):
    ...
    def _forward(self, tensordict: TensorDictBase) -> TensorDictBase:
        # === 调试代码 START ===
        # 这里的目的是在报错前拦截，看看缺了哪个 key
        try:
            for key in self.in_keys:
                val = tensordict.get(key, default=None)
                if val is None:
                    print(f"\n[DEBUG ERROR] MLP Model missing input key: '{key}'")
                    print(f"[DEBUG INFO] Available keys in current TensorDict: {list(tensordict.keys())}")
                    # 如果这是一个嵌套的 TensorDict (比如有 group 层级)，打印它的结构
                    if hasattr(tensordict, "root"):
                         print(f"[DEBUG INFO] Root keys: {list(tensordict.root.keys())}")
                    # 尝试打印 tensordict 的字符串表示，看大致结构
                    print(f"[DEBUG INFO] TensorDict structure:\n{tensordict}")
        except Exception as e:
            print(f"[DEBUG ERROR] Debug printing failed: {e}")
        # === 调试代码 END ===
